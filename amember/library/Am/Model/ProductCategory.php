...
class ProductCategoryTable extends Am_Table
{
    ...
    function getUserSelectOptions($options = [])
    {
        return $this->getSelectOptions(self::USER, $options);
    }
    function getAdminSelectOptions($options = [])
    {
    ...
    function getSelectOptions($type, array $options)
    {
        ...
        $scope = isset($options[self::SCOPE]) ? array_merge([-1], $options[self::SCOPE]) : null;
        $exclude_empty = !empty($options[self::EXCLUDE_EMPTY]);
        $exclude_hidden = !empty($options[self::EXCLUDE_HIDDEN]);
        $include_hidden = isset($options[self::INCLUDE_HIDDEN]) ? $options[self::INCLUDE_HIDDEN] : [];
    ...
    public function getProductIds($catId)
    {
        return $this->_db->selectCol(
            "SELECT product_id FROM ?_product_product_category WHERE product_category_id=?",
            $catId
        );
    }
}