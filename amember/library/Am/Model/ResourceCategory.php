...
class ResourceCategoryTable extends Am_Table
{
    ...
    function getAllowedTree(User $user)
    {
        ...
        $_ = $this->getAdapter()->selectCol("SELECT resource_category_id FROM ?_resource_resource_category;");
        $this->_resources_category_cnt = array_combine($_, array_fill(0, count($_), 0));

        $r_rc = $this->getAdapter()->selectCol(<<<CUT
                SELECT GROUP_CONCAT(resource_category_id), CONCAT(resource_id, '-', resource_type) AS ARRAY_KEY
                    FROM ?_resource_resource_category
                    GROUP BY ARRAY_KEY;
CUT
            );

        foreach($this->_resources as $k => $r) {
            $_ = isset($r_rc[$r->pk() . '-' . $r->getAccessType()]) ? explode(',', $r_rc[$r->pk() . '-' . $r->getAccessType()]) : [];
            foreach ($_ as $cat_id) {
                $this->_resources_category_cnt[$cat_id]++;
            }
        }
    ...
    {
        $res = $node->self_cnt = $this->_resources_category_cnt[$node->pk()] ?? 0;
