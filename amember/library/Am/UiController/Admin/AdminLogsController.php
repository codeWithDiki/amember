...
class AdminLogsController extends Am_Mvc_Controller_Pages
{
    ...
    public function createErrors($id, $title, $controller)
    {
        ...
        $g->addField(new Am_Grid_Field_Expandable('trace', ___('Trace')))
             ->setAjax($this->getDi()->url('admin-logs/get-error-trace?id={log_id}', null,false));

        $g->addField(new Am_Grid_Field_Expandable('referrer', ___('Referrer')))
            ->setPlaceholder(Am_Grid_Field_Expandable::PLACEHOLDER_SELF_TRUNCATE_BEGIN)
            ->setMaxLength(25);
//
//        $tr = array_flip(\Monolog\Logger::getLevels());
//        $g->addField('level', ___('Level'), true)
//            ->setRenderFunction(function($r, $fn, $g, $fo) use ($tr) {
//                if ($r->$fn == 300) {
//                    $bgcolor = '#fff9c4';
//                    $color ='#454430';
//                } else {
//                    $bgcolor = $r->$fn > 300 ? '#f3dcdc' : '#a5d6a7';
//                    $color = $r->$fn > 300 ? '#ba2727' : '#555';
//                }
//                return $g->renderTd(sprintf('<span style="background:%s; color:%s; padding:.4em; display:inline-block; font-size:.7rem">%s</span>',
//                    $bgcolor, $color, Am_Html::escape($tr[$r->$fn])), false);
//            });
...
CUT
    ...
    public function createAdminLog()
    {
        ...
        $g->setFilter(new Am_Grid_Filter_LogEntity('dattm', [
            'message' => 'LIKE',
            'record_id' => 'LIKE',
            'tablename' => 'LIKE',
            'ip' => '='
