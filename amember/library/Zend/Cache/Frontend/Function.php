...
class Zend_Cache_Frontend_Function extends Zend_Cache_Core
{
     ...
     *
     * Modified: by default it DO NOT CACHE OUTPUT <alex@cgi-central.net>
     *
    ...
    public function call($callback, array $parameters = array(), $tags = array(), $specificLifetime = false, $priority = 8)
    {
        ...
        $cacheOutput = !empty($tags['_cacheOutput']); // default - do not cache output

        if (empty($tags['id']))
            $id = $this->_makeId($callback, $parameters);
        else
            $id = $tags['id']; // added by alex@cgi-central - can specify fixed caching id
        if ( ($rs = $this->load($id)) && isset($rs[1]) && (count($rs)==2)) {
            // A cache is available
            $output = $cacheOutput ? $rs[0] : null;
            $return = $rs[1];
        } else {
    ...
    public function makeId($callback, array $args = array())
    {
        ...
        if (isset($object)) {
            try {
                if ($object instanceof \Closure) // added  by alex@cgi-central.net to support closure cache
                {
                    $c = new ReflectionFunction($object);
                    $tmp = $c->getFileName() . $c->getStartLine() . serialize($c->getStaticVariables());
